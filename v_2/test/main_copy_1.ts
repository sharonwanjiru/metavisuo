...
export class register_intern extends app.terminal implements mod.questionnaire {
	...
	check_table_inputs(): boolean {
		...
		const output: boolean =
			table_header && table_data === true ? true : false;
		//
	...
	check_simple_inputs(): boolean {
		...
		for (let input of inputs) {
			...
			if (input.dataset.dbname === undefined)
				throw new schema.mutall_error(
					`Database name for ${input} is missing`
				);
			//
			//Check for the ename
			if (input.dataset.ename === undefined)
				throw new schema.mutall_error(
					`Entity name for ${input} is missing`
				);
			//
			//Check for the column name
			if (input.name === undefined)
				throw new schema.mutall_error(
					`Column name for ${input} is missing`
				);
			//
	...
	*get_simple_inputs(): Generator<quest.label> {
	...
	validate_checkbox_inputs(unique: string): void {
		...
		if (!checked) {
			...
			const last: Array<HTMLInputElement> = this.checkboxes!.slice(-1);
			//
			//2.4. Create the error reporting panel at the end of the page
			last[0].parentElement!.nextElementSibling!.insertAdjacentHTML(
				"beforeend",
				`<div id="report">
		...
		show_errors.forEach(show => {
			...
			show.onclick = async () =>
				show.insertAdjacentHTML(
					"beforeend",
					`<dialog open>Select at least one value for the ${unique} from the ones listed above</dialog>`
				);
		});
		//
		//4.0. Collect all elements that close the error reporting dialog box
		const close_errors: Array<HTMLElement> = Array.from(
			this.document.querySelectorAll("#report>.close")
		);
		//

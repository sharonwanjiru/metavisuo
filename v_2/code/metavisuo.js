...
export class metavisuo extends page {
    ...
    async show_panels() {
        ...
        if (current_dbname) {
            //
            // Set the selector value to the curent one
            selector.value = current_dbname;
        }
        //
...
export class database extends schema.database {
    ...
    create_markers() {
        ...
        for (const key in paths)
            this.draw_marker(key, paths[key]);
    }
    //
    //Draw the named marker using the given path
    draw_marker(key, path_str) {
        //
...
class entity extends schema.entity {
    ...
    *collect_attributes() {
    ...
    draw_attributes() {
        ...
        const gattr = this.document.createElementNS(svgns, "g");
        // 

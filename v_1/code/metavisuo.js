...
export class metavisuo extends page {
    ...
    async show_panels() {
        ...
        const selector = this.populate_selector(databases);
        //
...
export class database extends schema.database {
    ...
    *collect_labels() {
    ...
    //
    //A method for reporting errors being drawn by the metavisio
    report_error() {
        //
        // Get the div element from the metavisio page that holds the report
        const report = this.view.get_element("report");
        //
        ...
        const dlg = this.view.get_element("error_report");
        //
    ...
    async load_x_y_positions() {
        ...
        //
        //Retrieve the data
        const result = await server.exec("database", [`${this.meta_dbname}`], "get_sql_data", [
            sql,
        ]);
        //
    ...
    *collect_relations(dbase) {
    ...
    entity_move(ev) {
        ...
        const position = this.entity_get_new_position(ev, group);
        //
...
class attribute extends schema.attribute {
    ...
    gtext) {
        ...
        this.visual.setAttribute("y", `${this.entity.y - this.entity.radius - 2 * index}`);
        //
        //Set the name of the label
        this.visual.textContent = this.name;
        //
        //Add apearances repesenting....
        const list = this.visual.classList;
...
(function (marker_1) {
    ...
    marker_1.arrow = arrow;
    // The code that is specific to the chickenfoot
    //
    class foot_optional extends root {
        //
        constructor(svg) {
            super(svg);
        }
    ...
    marker_1.foot_optional = foot_optional;
    //
    //
    class foot_optional_identifier extends root {
        //
        constructor(svg) {
            super(svg);
        }
    ...
    marker_1.foot_optional_identifier = foot_optional_identifier;
    class foot_mandatory extends root {
        //
        constructor(svg) {
            super(svg);
        }
    ...
    marker_1.foot_mandatory = foot_mandatory;
    //
    class foot_manda_identifier extends root {
        //
        constructor(svg) {
            super(svg);
        }
    ...
    marker_1.foot_manda_identifier = foot_manda_identifier;
    //The code that is specific to the line_tick_path
    class tick extends root {
        //
        constructor(svg) {
            super(svg);
        }
...
class entity extends schema.entity {
    ...
    *collect_attributes() {
    ...
    draw() {
        ...
        const text = this.draw_text(this.name, this.x, this.y);
        //
    ...
    draw_text(name, centerx, centery) {
        //
    ...
    draw_attributes() {
        //
        //Create a group tag for placing all our attributes.
        const gattr = this.view.document.createElementNS(svgns, "g");
        //
        ...
        // ["3,40" "5,36" "9,32"]
        const values = this.attributes.map((lables, i) => `${this.x}, ${this.y - this.radius - 2 * i}`);
        //
    ...
    select() {
        ...
        const previous = this.dbase.svg.querySelector(".selected");
        //
